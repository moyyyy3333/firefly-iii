...
class DisabledMFANotification extends Notification
{
    ...
    public function toMail(User $notifiable): MailMessage
    {
        ...
        return new MailMessage()
            ->markdown('emails.security.disabled-mfa', [
                'user'      => $this->user,
                'ip'        => $ip,
                'host'      => $host,
                'userAgent' => $userAgent,
    ...
    {
        return PushoverMessage::create((string) trans('email.disabled_mfa_slack', ['email' => $this->user->email]))->title((string) trans(
            'email.disabled_mfa_subject'
        ));
    }
    ...
    {
        return (string) trans('email.disabled_mfa_slack', ['email' => $this->user->email]);
    }
